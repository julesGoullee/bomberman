...
module.exports =  function Maps(scene, menuPlayers) {
    ...
    for (var iBlockLargeur = -_colLength / 2; iBlockLargeur <= _colLength / 2; iBlockLargeur++) {
      ...
      for (var iBlockLongueur = -_lineLength / 2; iBlockLongueur <= _lineLength / 2; iBlockLongueur++) {
        ...
        for (iBlockTemp; iBlockTemp < tempBlock.length; iBlockTemp++) {
          ...
          if (currentBlock.position.x === iBlockLargeur * cfg.blockDim && currentBlock.position.z === iBlockLongueur * cfg.blockDim) {
        ...
        for (iBlockPerm; iBlockPerm < _blocksPermanent.length; iBlockPerm++) {
          ...
          if (currentBlock.x === iBlockLargeur * cfg.blockDim && currentBlock.z === iBlockLongueur * cfg.blockDim) {
  ...
  function createGroundMeshs() {
    for (var iMesh = 0; iMesh < self.meshsData.length; iMesh++) {
      ...
      var mesh = Assets.get(self.meshsData[iMesh].name)[0];
      ...
      if (self.meshsData[iMesh].colisionCase) {
        ...
        var meshColision = Assets.get(self.meshsData[iMesh].name + 'Colision')[0];
        ...
        if (iBlockLargeur % 2 === 0) {
          ...
          if (iBlockLongueur % 2 !== 0) {
            ...
            _blocksPermanent.push({
              ...
              x: iBlockLargeur * cfg.blockDim,

              z: iBlockLongueur * cfg.blockDim
            });
          }
        }
      }
  ...
  function createTemporaireBlock(blocksTemp) {
    ...
    for (var i = 0; i < blocksTemp.length; i++) {
      ...
      _content.push(new Block(blockTemp.id, blockTemp.position));
    }
  }
  ...
  function createPowerUp() {
    ...
    if (blocks.length >= cfg.nbPowerUp) {
      for (var i = 0; i < cfg.nbPowerUp; i++) {
        ...
        _powerUps.push(new PowerUp(position, '', ''));
    ...
    for (var i = 0; i < players.length; i++) {
      ...
      for (var j = 0; j < powerUps.length; j++) {
        ...
        if (players[i].meshs.shape.intersectsMesh(powerUps[j].meshs.shape, false)) {
          ...
          (function (i) {
            scene.beginAnimation(players[i].meshs.shape, 60, 110, false, 2, function () {
            });
          })(i);
